 ...
 * as an array of Java <code>short</code> type.
 *
 * Note that this technique only works with serial GC (used on PS5).
 */
public class ShortViewPointer extends Pointer {
    private static final long serialVersionUID = 4698355706655405326L;

    private final short[][] dataViewPtr;

    public ShortViewPointer(int length) {
        super(UNSAFE.allocateMemory(length * Unsafe.ARRAY_SHORT_INDEX_SCALE + Unsafe.ARRAY_SHORT_BASE_OFFSET) + Unsafe.ARRAY_SHORT_BASE_OFFSET,
                new Long(length * Unsafe.ARRAY_SHORT_INDEX_SCALE));

        // Obtain klass value for the array type.
        short[] arrayKlassInstance = new short[0];
        ...
        dataViewPtr = new short[1][0];
    }

    @Override
    public void free() {
        // Make sure to free the memory starting at the simulated header
        UNSAFE.freeMemory(this.addr - Unsafe.ARRAY_SHORT_BASE_OFFSET);
    }

    /**
     * Returns a simulated view of the pointer data as a Java native array of shorts.
     ...
     * <pre>
     *     short[] dataView = shortViewPointer.dataView();
     *     dataView[3] = 0x1234;
     *     dataView = null;
     * </pre>
     ...
     */
    public short[] dataView() {
        Pointer dataViewPtrAddr = Pointer.valueOf(addrOf(dataViewPtr));
        dataViewPtrAddr.write8(Unsafe.ARRAY_OBJECT_BASE_OFFSET, this.addr - Unsafe.ARRAY_SHORT_BASE_OFFSET);
        short[] dataView = dataViewPtr[0];
