 ...
 * as an array of Java <code>byte</code> type.
 *
 * Note that this technique only works with serial GC (used on PS5).
 */
public class ByteViewPointer extends Pointer {
    private static final long serialVersionUID = 4698355706655405326L;

    private final byte[][] dataViewPtr;

    public ByteViewPointer(int length) {
        super(UNSAFE.allocateMemory(length * Unsafe.ARRAY_BYTE_INDEX_SCALE + Unsafe.ARRAY_BYTE_BASE_OFFSET) + Unsafe.ARRAY_BYTE_BASE_OFFSET,
                new Long(length * Unsafe.ARRAY_BYTE_INDEX_SCALE));

        // Obtain klass value for the array type.
        byte[] arrayKlassInstance = new byte[0];
        ...
        dataViewPtr = new byte[1][0];
    }

    @Override
    public void free() {
        // Make sure to free the memory starting at the simulated header
        UNSAFE.freeMemory(this.addr - Unsafe.ARRAY_BYTE_BASE_OFFSET);
    }

    /**
     * Returns a simulated view of the pointer data as a Java native array of bytes.
     ...
     * <pre>
     *     byte[] dataView = byteViewPointer.dataView();
     *     dataView[3] = 0x12;
     *     dataView = null;
     * </pre>
